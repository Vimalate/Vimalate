...
三次握手是为了```确保客户端和服务端双方的接收与发送能力是否正常```，至于为什么不是不是两次，四次，是因为为了避免不必要的性能的开销,而3次，是完成双方考察发送和接收能力的最少次数

为什么四次挥手

由于 tcp 是双向的，当我们客户端发送给服务端所可以断开连接了，但这个时候我们服务端数据可能还没发送完，所以需要进行一次确认。我们服务端发送一个**我收到了的消息（ACK确认包）**给客户端，然后服务端再发送一个**我已经传输完毕了的消息（FIN包），可以断开链接了**，然后客户端确认到关闭端请求，给服务端发送收到端消息，双方就关闭了
